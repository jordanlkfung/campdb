...
function RegisterCamp(){
    ...
    return(
        <div className="container mx-auto md:p8 mt-36 border-2 p-9 space-x-3">
            ...
            <select onChange={(e)=>setCamp({...registerCamp,child_id:e.target.id})} className="w-1/3 p-1">
                {getChild.map((child)=>{
                    return <option id={child.id} value={child.first_name}>{child.first_name+' '+child.last_name}</option>
                })}
            </select>
            <select onChange={(e)=>setCamp({...registerCamp,camp_id:e.target.id})} className="w-1/3 p-1">
                {camps.map((camp)=>{
